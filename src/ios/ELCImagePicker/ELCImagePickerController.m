...
- (BOOL)shouldSelectAsset:(ELCAsset *)asset previousCount:(NSUInteger)previousCount
{
    ...
    if (!shouldSelect) {
        NSString *title = [NSString stringWithFormat:NSLocalizedString(@"最多只能选择 %d 张照片.", nil), self.maximumImagesCount];
        NSString *message = [NSString stringWithFormat:NSLocalizedString(@"最多只能选择 %d 张照片.", nil), self.maximumImagesCount];
